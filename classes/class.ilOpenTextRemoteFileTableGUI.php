...
class ilOpenTextRemoteFileTableGUI extends ilTable2GUI
{
    ...
    public function fillRow($a_set)
    {
        ...
        $this->tpl->setVariable('SKYDOC_LINK', \ilOpenTextUtils::getInstance()->generateOpenTextDirectLink($a_set['id']));
        $this->tpl->setVariable('SKYDOC_NAME', $a_set['id']);

        $refs = $this->getReferences($a_set['obj_id']);
        if (!count($refs)) {
            $this->tpl->setCurrentBlock('title');
            $this->tpl->setVariable('OBJ_TITLE', $a_set['name']);
            ...
            $this->tpl->setCurrentBlock('title');
            $this->tpl->setVariable('OBJ_LINKED_TITLE', $a_set['name']);
        ...
        $this->tpl->setVariable('CDATE', \ilDatePresentation::formatDate(new \ilDateTime($a_set['create_date'], IL_CAL_UNIX)));
        ...
        if ((int) $a_set['id'] > 0) {
            ...
            $selection->setId('sync_item_' . $a_set['id']);
            $selection->setListTitle($this->lng->txt('actions'));

            $this->ctrl->setParameter($this->getParentObject(), 'id', $a_set['id']);
            $selection->addItem(
                $this->plugin->txt('open_in_opentext'),
                '',
                \ilOpenTextUtils::getInstance()->generateOpenTextDirectLink($a_set['id']),
                '',
                '',
                '_blank'
            );
     ...
     * @return array|\Swagger\Client\Model\ResultsData[]
     */
